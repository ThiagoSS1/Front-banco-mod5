...
function adicionarRecado ( description, details) {
  ...
  if (!!!descriptionInput && !!!detailsInput) {
    ...
    await api.post(`/messages/${JSON.parse(id)}`).then((response) => {
...
async function preencherLinha () {
  ...
  const id = localStorage.getItem("userId");
  // Criar recados
  const recados = await api.get(`/messages/${JSON.parse(id)}`).then((response) => {
    return response.data.data
  })
